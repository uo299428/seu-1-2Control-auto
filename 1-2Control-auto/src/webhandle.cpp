...
char *readTextFile(const char * path, int max_len){
    ...
    // Read file
    file.readBytes(str, MAX_STR_LEN);
    str[nbytes] = '\0';
...
void replace_patterns_html_sta(String &str)
{
    ...
    sprintf(temp_str, "%2.1f", getTemperature());
    sprintf(hum_str, "%2.0f", getHumidity());
    ...
    str.replace("@dev_name@", "Ramsés Diaz");
...
void handleRoot_sta() 
{
    ...
    if (server.args() > 0)
    {
        ...
        for (int i = 0; i < server.args(); i++) 
        {
            ...
            else if (server.argName(i).equals("control_mode")) //preguntar esta condicion
            {
